...
*   **Dynamic OSM Overpass API Integration:** Rather than hardcoding static mock clinic locations, the app issues read-only Overpass QL POST requests (`https://overpass-api.de/api/interpreter`) targeting nearby `hospital` or `pharmacy` amenity elements inside a 4km radius around the active browser geolocation, sorting results via the Haversine formula and displaying top results on the Leaflet map.
